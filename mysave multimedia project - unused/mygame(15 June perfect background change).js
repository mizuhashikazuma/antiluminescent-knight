...
scene("mygame",  ({ level, score }) =>{
	...
	layer(['bg', 'obj', 'ui'], 'bg')
	// const background = add([sprite('bg1'), scale(0.957), layer('bg')])
	...
	const levelCfg = {
		...
		"?": () => [sprite('bg1'), scale(0.957),],
		'+': () => [sprite('bg2'), scale(2), layer('bg')],
		'x': () => [sprite('bg3'), scale(2), layer('bg')],

		

	    // '?': [const background('bg'), scale(0.957), layer('bg')],
	    // '?': [sprite('bg'), scale(0.957), layer('bg')],
	    // '+': [background.changeSprite('loading')],
	    // 'x': [background.changeSprite('tree1')],
	    // '+': [background.changeSprite('loading')],
	    // 'x': [background.changeSprite('tree1')],
...
scene("lose", ({score}) =>{
	...
	wait(3, () =>{
		add([text('Press ENTER to restart'), pos(635,650), scale(1.5)])
		keyPress('enter', () => go("mygame",{ level:0, score:0}))

		add([text('Press ESC to quit'), pos(662,665), scale(1.5)])
	...
	// add([text('Press ENTER to restart'), pos(630,650), scale(1.5)])
	// keyPress('enter', () => go("mygame",{ level:0, score:0}))
