...
loadSprite('tryagain', 'image/tryagainbutton.png')
...
scene("lose", ({score}) =>{
	...
	add([text('Game Over!'), pos(570,200), scale(6)])
...
scene("win", ({score}) =>{
    ...
    victorysong.play()
    const victorybg = add([sprite('victorybg'), scale(1.2),])
	add([text(score, 32), origin('center'), pos(width()/2, height()/2), color(0,0,0)])
	add([text('Congratulation!! U save the world'), pos(400,150), scale(3), color(0,0,0)])
